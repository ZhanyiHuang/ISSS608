...
```{r}
# Import Coffee Chain data on Mac using mdbtools (RODBC is Windows-only)
library(dplyr)
library(lubridate)
library(readr)

mdb_path <- "/Users/zhanyi/Desktop/M Y1S2/ISSS608/ISSS608-VAA/project 1/ISSS608/Hands-on_Ex/Hands-on_Ex10/data/Coffee Chain.mdb"
data_dir <- "/Users/zhanyi/Desktop/M Y1S2/ISSS608/ISSS608-VAA/project 1/ISSS608/Hands-on_Ex/Hands-on_Ex10/data"

# Export the relevant tables from the Access database to CSV
system(paste0('mdb-export "', mdb_path, '" factTable > "', data_dir, '/factTable.csv"'))
system(paste0('mdb-export "', mdb_path, '" Product > "', data_dir, '/Product.csv"'))

# Read both tables into R
factTable <- read_csv(file.path(data_dir, "factTable.csv"))
product_lookup <- read_csv(file.path(data_dir, "Product.csv"))

# Join fact table with product lookup to bring in Product names, and parse Date
coffeechain <- factTable %>%
  left_join(product_lookup, by = "ProductId") %>%
  mutate(Date = mdy_hms(Date))

# Save as .rds so this import step doesn't need to be repeated
write_rds(coffeechain, file.path(data_dir, "CoffeeChain.rds"))
```
...
```{r}
coffeechain <- read_rds("/Users/zhanyi/Desktop/M Y1S2/ISSS608/ISSS608-VAA/project 1/ISSS608/Hands-on_Ex/Hands-on_Ex10/data/CoffeeChain.rds")
```
...
In this section, you will learn how to create static information dashboard by using gt and gtExtras packages. Before getting started, it is highly recommended for you to visit the webpage of these two packages and review all the materials provided on the webpages at least once. You do not have to understand and remember everything provided but at least have an overview of the purposes and functions provided by them.
...
It is important to note that one of the requirements of gtExtras functions is that almost exclusively they require you to pass data.frame with list columns. In view of this, code chunk below will be used to convert the report data.frame into list columns.
